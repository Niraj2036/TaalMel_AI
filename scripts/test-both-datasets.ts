...
async function testBothDatasets() {
  console.log('====================================================');
  console.log('       TESTING ENGINE ON BOTH DATASETS              ');
  console.log('====================================================\n');
  ...
  // --- DATASET 1 (data/new/) ---
  {
    ...
    const result = reconcile(erpTxns, bankTxns, gatewayTxns, 'test-dataset-1');

    const matchedBankIds = new Set(result.matches.flatMap(m => m.bankTxnIds));
    console.log('--- DATASET 1 RESULTS (Expected: 132/135 matched = 97.8%) ---');
    console.log(`Bank Records Total: ${bankTxns.length}`);
    console.log(`Matched Bank Credits: ${matchedBankIds.size}`);
    console.log(`Match Rate: ${result.metrics.matchRate}%`);
    console.log(`Unmatched Bank Credits: ${bankTxns.length - matchedBankIds.size}`);
    console.log(`Exceptions Total: ${result.exceptions.length}\n`);
  }
  ...
  // --- DATASET 2 (data/dataset2/) ---
  {
    ...
    const matchedBankIds = new Set(result.matches.flatMap(m => m.bankTxnIds));
    console.log('--- DATASET 2 RESULTS (Expected: 263/287 matched = 91.64%) ---');
    console.log(`Bank Records Total: ${bankTxns.length}`);
    console.log(`Matched Bank Credits: ${matchedBankIds.size}`);
    console.log(`Match Rate: ${result.metrics.matchRate}%`);
    console.log(`Unmatched Bank Credits: ${bankTxns.length - matchedBankIds.size}`);
...
testBothDatasets().catch(console.error);
